...
async def stats_command(m: UpdateNewMessage):
    if m.is_group or m.is_channel:
        return
    check_if = await is_user_on_chat(bot, f"@storm_core", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @storm_core then send me the link again.")
    check_if = await is_user_on_chat(bot, f"@BillaSpace", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @BillaSpace then send me the link again.")
...
"""
    ...
    if m.sender_id == OWNER_ID:
        ...
        await m.reply("Aww,This Command Is Only For My Owner.")
...
**Just send me the Terabox link** and I'll start downloading it for you Baby.
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Update Channel", "https://t.me/BillaSpace"),
                Button.url(
                    "Coder", "https://t.me/x_ifeelram"
                ),
            ]
        ],
        link_preview=False,
...
async def help_command(m: UpdateNewMessage):
    ...
    if not check_if:
        return await m.reply(f"Please join @BillaSpace then send me the link again.")
    check_if = await is_user_on_chat(bot, f"@storm_techh", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @storm_techh then send me the link again.")
    help_text = """
Available commands:
...
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Updates", "https://t.me/BillaSpace"),
                Button.url(
                    "DEV", "https://t.me/x_ifeelram"
                ),
            ]
        ],
    )
...
async def handle_message(m: Message):
    ...
    check_if = await is_user_on_chat(bot, "@BillaNothing", m.peer_id)
    if not check_if:
        return await m.reply("Please join @BillaNothing  then send me the link again.")
    check_if = await is_user_on_chat(bot, "@BillaSpace", m.peer_id)
    if not check_if:
        return await m.reply("Please join @BillaSpace then send me the link again.")
...
print("This bot is deployed by @BillaSpace kindly join this channel for more updates.")
